...
//         id && $store.dispatch('declarationForm', id),
//       ];

//       Promise.all(promises).then(() => {
//         let declaration = id ? $store.getters.declarationForm : {};
//         declaration = { ...declaration.foret, ...declaration };

//         ///
//         // this.declaration = configDeclaration.initModel(declaration);

//         declaration.id_declarant = declaration.id_declarant || $store.getters.user.id_role;
...
//   },
//   title: ({ id }) =>
//     id ? `Modifier la déclaration ${id}` : 'Signaler des dégâts de grand gibier en forêt',
//   formDefs,
//   sessionDefs,
//   sessionGroups: {
//     foret: {
