...
app.config["URL_REDIRECT"] = (
    app.config["URL_FRONTEND"] + app.config["REDIRECT_ON_FORBIDDEN"]
)
...
migrate.init_app(
    app, db, directory=Path(__file__).absolute().parent / "oeasc" / "migrations"
)
...
def configure_alembic(alembic_config):
    ...
    if "ALEMBIC_VERSION_LOCATIONS" in current_app.config:
        version_locations.extend(
            current_app.config["ALEMBIC_VERSION_LOCATIONS"].split()
        )
