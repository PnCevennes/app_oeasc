...
SHAPETYPE = {"POINT": 1, "POLYLINE": 3, "POLYGON": 5}
...
def create_shapes(db_cols, columns, data, dir_path, file_name, geom_col, srid):
    ...
    zip_it(dir_path, file_name, ["POINT", "POLYLINE", "POLYGON"])


def create_shapes_generic(
    mapped_table, db_cols, columns, data, dir_path, file_name, geom_col, srid
):
    point, line, polygon = create_shape_struct(db_cols, columns)
    for d in data:
