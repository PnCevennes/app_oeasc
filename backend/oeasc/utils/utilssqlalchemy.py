...
# from oeasc.utils.utilsshapefile import create_shapes_generic

# from oeasc.utils.errors import GeonatureApiError
...
#         feature = Feature(geometry=geometry, properties=self.as_dict(data, columns))
#         return feature
...
#                 orderCol = getattr(self.view.tableDef.columns, parameters["orderby"])
#         else:
#             return query
...
#             results = FeatureCollection([self.view.as_geofeature(d) for d in data])
#         else:
...
#         (db_rel.key, db_rel.uselist, db_rel.argument)
...
# def tojson_resp(res, status=200, filename=None, as_file=False, indent=None):
#     if not res:
#         status = 404
