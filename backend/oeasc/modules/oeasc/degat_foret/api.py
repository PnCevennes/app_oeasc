...
def api_get_declaration(id_declaration):
    ...
    for key in out:
        if (
            isinstance(out[key], list)
            and len(out[key]) > 0
            and "id_nomenclature" in out[key][0]
        ):
    ...
    #   foret
    areas_foret = [
        get_area_from_id(area["id_area"]) for area in out.get("areas_foret", [])
    ]
    ...
    #   declaration
    areas_localisation = [
        get_area_from_id(area["id_area"]) for area in out.get("areas_localisation", [])
    ]
    out["areas_localisation_cadastre"] = get_id_areas(
        areas_localisation, ["OEASC_CADASTRE"]
    )
    out["areas_localisation_onf_prf"] = get_id_areas(
        areas_localisation, ["OEASC_ONF_PRF"]
    )
    out["areas_localisation_onf_ug"] = get_id_areas(
