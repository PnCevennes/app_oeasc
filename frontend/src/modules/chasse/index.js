...
const ROUTE = [
  ...
  // j'ai repassé la page bilan données en composant classique (au lieu d'une page dynamique) car il y avait des défauts
  // comme le fait que des requêtes api étaient refaites plusieurs fois.
  {
    name: 'chasse.bilan',
    path: '/chasse/bilan',
    label: 'Bilan données chasse', // titre dans le menu (mais pas dans la page)
...
const chasseAction =
  ...
  (
    { getter },
    { id_saison, id_espece, id_zone_cynegetique, id_zone_indicative, id_secteur, poids_ou_dagues }
  ) => {
    getter;

    return apiRequest('GET', `api/chasse/results/${actionType}`, {
      params: {
        id_saison,
        id_espece,
        id_secteur,
...
const STORE = {
  getters: {
    /**
     * Génère la config d'un formulaire de filtres pour la plupart des pages de chasse (analyse détaillée, bilan données, plan de chasse )
     * @param {Array<string>} fields - Liste des noms de champs à inclure dans le formulaire (ex: ['id_saison', 'id_espece', 'id_secteur']).
  ...
  actions: {
    ...
    chasseEchelle: ($store, params) => {
      ...
      return new Promise((resolve) => {
        for (const idFieldName of ['id_zone_indicative', 'id_zone_cynegetique', 'id_secteur']) {
          ...
          $store.dispatch(configStore.getAll, params).then((val) => {
            resolve(
              `${configStore.labels} : ${val.map((v) => v[configStore.displayFieldName]).join(', ')}`
            );
          });
          return;
        }
    ...
    chasseLastTauxRealisation: ({ getter }, params) => {
      return new Promise((resolve) =>
        chasseAction('bilan')({ getter }, params).then((bilan) => {
...
// liste des composants susceptibles d'être utilisés dans les pages créées dynamiquement.
const CONTENT = {
  Graph_bilan_evolution, // utilisé dans plan de chasse
  graphChasseIce, // utilisé dans indice de performance
