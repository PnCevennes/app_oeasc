...
export default {
  ...
  methods: {
    ...
    onSuccess(event) {
      ...
      if (
        !this.realisations.find(
          (r) => r.attribution.numero_bracelet == event.attribution.numero_bracelet
        )
      ) {
      ...
      // sinon c'est un update : on modifie la réalisation concernée
        ...
        this.realisations = this.realisations.map((r) =>
          r.attribution.numero_bracelet == event.attribution.numero_bracelet ? event : r
        );
      }
