...
export default {
  ...
  form: {
    groups: [
      ...
      {
        title: 'Localisation',
        condition: ({ baseModel }) => baseModel.attribution && baseModel.attribution.id_attribution,
        groups: [
          {
            direction: 'row',
            groups: [
      ...
      // Groupe "Informations" : affiché si une attribution est sélectionnée
      {
        ...
        groups: [
          ...
          {
            title: 'Dates',
            forms: ['date_exacte_fast', 'date_exacte', 'date_enreg_fast', 'date_enreg'],
          },
          {
            title: 'Compléments',
  ...
  defs: {
    ...
    attribution: {
      ...
      change: ({ baseModel, $store }) => {
        ...
        baseModel.zone_cynegetique_affectee =
          baseModel.zone_cynegetique_affectee || baseModel.attribution.zone_cynegetique_affectee;

        baseModel.zone_cynegetique_realisee =
          baseModel.zone_cynegetique_realisee || baseModel.attribution.zone_cynegetique_affectee;

        baseModel.zone_indicative_affectee =
          baseModel.zone_indicative_affectee || baseModel.attribution.zone_indicative_affectee;

        baseModel.zone_indicative_realisee =
          baseModel.zone_indicative_realisee || baseModel.attribution.zone_indicative_affectee;
    ...
    date_exacte_fast: {
      ...
      rules: ({ baseModel }) =>
        baseModel.saison && [
          (v) => (v && v.length == 4) || 'la date doit être au format "JJMM" (4 caractères)',
          (v) => {
            const d = formFunctions.getDateFromMMJJ(
              v,
              baseModel.saison.date_debut,
    ...
    date_enreg_fast: {
      ...
      rules: ({ baseModel }) =>
        baseModel.saison && [
          (v) => (v && v.length == 4) || 'la date doit être au format "JJMM" (4 caractères)',
          (v) => {
            const dateMin = `${baseModel.saison.date_debut.substring(0, 4)}-01-01`;
            const dateMax = `${baseModel.saison.date_fin.substring(0, 4)}-12-31`;
    ...
    date_exacte: {
      ...
      required: true,
      condition: ({ baseModel }) => {
        return !baseModel || !!baseModel.date_exacte;
      },
    ...
    gestation: {
      ...
      type: 'bool_switch',
      disabled: ({ baseModel }) =>
        baseModel.nomenclature_sexe && baseModel.nomenclature_sexe.label_fr != 'Femelle',
    },
