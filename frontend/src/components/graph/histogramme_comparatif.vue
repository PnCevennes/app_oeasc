...
export default {
      ...
      chartOptions: {
        ...
        tooltip: {
          ...
          formatter: function () {
            ...
            let s = `<b>${this.x}</b><br/>`;
            const chart = (this.points && this.points[0] && this.points[0].series && this.points[0].series.chart)
              ? this.points[0].series.chart
              : null;
            const referenceLabel = (chart && chart.options && chart.options.custom && chart.options.custom.referenceLabel)
  ...
  methods: {
    ...
    creation_serie_highcharts: (data, props) => {
        ...
        const hasReference = typeof props.fieldReference === 'string' && props.fieldReference.trim() !== '';
        ...
        if (hasReference && hasValues) {
          ...
          const compareSeries = props.listfieldValues.map((name_field, index) => {
            const label = props.listNameValues && props.listNameValues[index]
          ...
          const restantSeries = {
            ...
            hideInTooltip: true,
            hideDataLabels: true,
            color: props.code_couleurs && (props.code_couleurs.restant || props.code_couleurs[props.fieldReference])
