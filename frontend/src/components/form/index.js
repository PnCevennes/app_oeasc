  ...
  actions: {
    ...
    setClearableTabIndex: ({ state }, selector) => {
      ...
      setTimeout(() => {
        ...
        const elem = selector ? document.querySelector(selector) : document;
        ...
        elem.querySelectorAll('.v-input__icon--clear, .mdi-close').forEach((elem) => {
          // Pour chaque icône, on cherche les boutons enfants et on leur attribue tabindex -1.
          Array.from(elem.getElementsByTagName('button')).forEach((b) => {
            b.tabIndex = -1;
          });
        });
