      ...
      <div class="list-form">
        ...
        <div v-else-if="config.list_type === 'combobox'">
          <v-combobox
            ...
            :item-title="config.displayFieldName"
            :rules="config.rules"
        ...
        <div v-else>
          ...
          <div v-if="config.multiple">
            <v-container fluid>
              <div
                class="select-list-label-container"
                v-if="!!config.label"
              >
...
export default {
  ...
  methods: {
          ...
          values = values
            .map((value) => {
              if (typeof value === 'string') {
                const elem = this.items.find((item) => item[this.config.displayFieldName] == value);
                if (elem) {
                  return elem;
                }
              ...
              const index = self.findIndex(
                (e) =>
                  (e[this.config.valueFieldName] &&
                    e[this.config.valueFieldName] == item[this.config.valueFieldName]) ||
                  e[this.config.displayFieldName] == item[this.config.displayFieldName]
              );
              return index == pos;
            });
          // Mise à jour du modèle avec la nouvelle sélection
          this.baseModel[this.config.name] = this.config.multiple ? values : values[0];
        }
      }
    ...
    clickChips() {
      setTimeout(() => {
        const chips =
          this.$refs.autocomplete && this.$refs.autocomplete.$el.getElementsByClassName('v-chip');
    ...
    setDefaultConfig() {
      ...
      if (this.config.storeName) {
        ...
        this.defaultConfig.valueFieldName = this.config.valueFieldName || configStore.idFieldName;
    ...
    getData: function () {
      ...
      // Si un store est défini dans la configuration
        ...
        this.config.valueFieldName = this.config.valueFieldName || configStore.idFieldName;
        this.config.displayFieldName = this.config.displayFieldName || configStore.displayFieldName;
        // Tri de la colonne d'affichage
        this.config.displayFieldSortDesc = this.config.displayFieldSortDesc || false;
