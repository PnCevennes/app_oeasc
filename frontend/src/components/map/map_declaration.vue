...
<template>
    <div style=" width: 100%; display: flex; flex-direction: row;">
      ...
      <div style="position: relative; width: 70%;">
        ...
        >
          <v-icon start>mdi-regular mdi-reply</v-icon>
          {{ typesCarteNames[last_typeCarte] || "Retour" }}
        </v-btn>
        <div id="map" style="height: 600px; min-width: 500px; z-index: 0;"></div>
        <div
          v-if="freeze_map"
          style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; z-index: 10; background: rgba(255,255,255,0.3); cursor: progress; display: flex; flex-direction: column; align-items: center; justify-content: center;"
        >

        <v-progress-circular indeterminate color="primary" size="150" width="10"
            :model-value="geojsonVisibleLayers" ></v-progress-circular>
  ...
  export default {
    ...
    computed: {
      areasLocalisationOfThisMap() {
        if( this.declaration_data.b_document ===true && this.declaration_data.b_statut_public === true){
          return (this.declaration_data.areas_localisation || []).filter(area => area.id_type === UG_ONF);
        }else{
          return (this.declaration_data.areas_localisation || []).filter(area => area.id_type === CADASTRES);
        }
    ...
    methods: {
      ...
      /**
       * Affiche la carte par défaut avec les périmètres sélectionnés
       * et met le type de carte à DEFAUT pour permettre au bouton retour de tout réinitialiser.
       */
      affichage_map_zones_selectionnees() {
        // Affiche la carte par défaut avec les périmètres sélectionnés dans list_selected_areas
        if (!this.list_selected_areas || this.list_selected_areas.length === 0) return;

        // Récupérer les données GeoJSON pour les zones sélectionnées
        if (this.list_selected_areas.length !== 0) {
          
            fetch_areas_from_list_ids(this.list_selected_areas).then((geojson) => { 
              this.geojsonVisibleLayers = geojson; // Met à jour les données GeoJSON visibles
              // this.add_layers_on_map(); // Ajoute les couches de la carte avec les périmètres sélectionnés
            }).catch((error) => {
              console.error("Erreur lors de la récupération des zones sélectionnées :", error);
            });
        }
        this.typeCarte = DEFAUT; // Met le type de carte à par défaut
      },
      ...
      change_type_carte_after_click() {
        ...
        if (this.typeCarte === COMMUNES) { // on était sur les communes, on a donc cliqué sur une section
      ...
      async add_layers_on_map() {
        // Affiche les layers d'ensemble à la carte. C'est à dire les communes, sections, les forêts DGD et ONF. Cliquer dessus
       ...
       */
      render_clickable_layers() {
        // Affiche les couches cliquables sur la carte
        const vm = this;
        const map_layers = L.geoJSON(this.geojsonVisibleLayers, {
            style: styles.normal,
            onEachFeature: (feature, layer) => {
                layer.bindTooltip(feature.properties.area_name || "", { // options pour l'affichage du nom de la commune
                    direction: "top",
                    offset: [0, -20],
                ...
                layer.on("mouseout", function () {
                    layer.setStyle(
                      vm.declaration_data.areas_localisation.some(area => area.id_area === feature.properties.id_area)
                      ? styles.select
                      : styles.normal
                    );
                });
                layer.on("click", () => {
                    ...
                    this.action_click_on_layer(feature, layer); // Gère le clic sur la couche
                });
                layer.setStyle(
                      this.declaration_data.areas_localisation.some(area => area.id_area === feature.properties.id_area)
                      ? styles.select
                      : styles.normal
                    );
            },
       ...
       */
      render_selectable_layers(){
        // ajoute les layers de type cadastres. Cliquer dessus les rajoute à la liste des périmètres sélectionnés
        // Affiche les couches cliquables sur la carte
        ...
        const map_layers = L.geoJSON(this.geojsonVisibleLayers, {
            ...
            onEachFeature: (feature, layer) => {
                ...
                // Met à jour dynamiquement le style selon la sélection
                layer.setStyle(
                      vm.declaration_data.areas_localisation.some(area => area.id_area === feature.properties.id_area)
                      ? styles.select
                      : styles.normal
      ...
      change_style_layer(properties_layer, style) {
        ...
        this.map.eachLayer(l => {
          if (l.feature && l.feature.properties && l.feature.properties.id_area === properties_layer.id_area) {
            foundLayer = l;
          }
        });
        if (foundLayer) {
      ...
      select_area(feature){
          ...
          if (this.declaration_data.b_document == false) { 
                ...
                if (!this.declaration_data.areas_localisation.some(area => area.id_area === area_commune.id_area)) {
                  this.declaration_data.areas_localisation.push(area_commune);
                }
                if (!this.declaration_data.areas_localisation.some(area => area.id_area === area_section.id_area)) {
            ...
            // ################## CAS DES FORET DGD ##############################
              if (this.declaration_data.b_statut_public == false){
                if (this.declaration_data.areas_foret_dgd !== this.actual_foret_dgd) { // Si la forêt DGD sélectionnée est différente de celle déjà sélectionnée, on efface tout car il ne peut y avoir qu'une foret selectionné
                  this.reset_areas(); // Réinitialise les zones sélectionnées
                  ...
                  this.declaration_data.code_foret = this.liste_areas_selected[this.actual_foret_dgd].area_code; 
                  this.declaration_data.surface_renseignee = this.liste_areas_selected[this.actual_foret_dgd].surface_renseignee;
                  this.declaration_data.surface_calculee = this.liste_areas_selected[this.actual_foret_dgd].surface_calculee;
                }
                this.declaration_data.areas_localisation_cadastre.push(feature.id_area);
                let area_dgd = this.liste_areas_selected[this.actual_foret_dgd];
                area_dgd['id_parent'] = null;
                if (!this.declaration_data.areas_localisation.some(area => area.id_area === area_dgd.id_area)) {
                  this.declaration_data.areas_localisation.push(area_dgd);
                }
                this.declaration_data.areas_localisation.push({...feature, id_parent: this.actual_foret_dgd});
                  ...
                  if (this.declaration_data.areas_foret_onf !== this.actual_foret_onf) { 
                    ...
                    this.declaration_data.code_foret = this.liste_areas_selected[this.actual_foret_onf].area_code; 
                    this.declaration_data.surface_renseignee = this.liste_areas_selected[this.actual_foret_onf].surface_renseignee;
                    this.declaration_data.surface_calculee = this.liste_areas_selected[this.actual_foret_onf].surface_calculee;
                  }
                  ...
                  if (!this.declaration_data.areas_localisation.some(area => area.id_area === area_onf.id_area)) {
                    this.declaration_data.areas_localisation.push(area_onf);
                  }
                  if (!this.declaration_data.areas_localisation.some(area => area.id_area === area_onf_parcelle.id_area)) {
                    this.declaration_data.areas_localisation.push(area_onf_parcelle);
                  }
                  if (!this.declaration_data.areas_localisation.some(area => area.id_area === area_onf_ug.id_area)) {
      ...
      unselect_area(feature){
          ...
          this.declaration_data.areas_localisation = this.declaration_data.areas_localisation.filter(area => area.id_area !== feature.id_area); 
          
          if (this.declaration_data.b_document == false) { // Si le statut public est faux, on est en mode DGD ou sections
            // ################## CAS DES SECTIONS ET COMMUNES ##################
            this.declaration_data.areas_localisation_cadastre.pop(feature.id_area); // Retire l'id de la zone cadastrale de la déclaration
            ...
            if (!this.declaration_data.areas_localisation.some(area => area.id_parent === this_parent)) {
              this.declaration_data.areas_foret_sections.pop(this_parent); // Retire la section de la liste des zones sélectionnées
              ...
              this.declaration_data.areas_localisation = this.declaration_data.areas_localisation.filter(area => area.id_area !== this_parent);
                ...
                this.declaration_data.areas_localisation = this.declaration_data.areas_localisation.filter(area => {
                  return area.id_parent == null || this.declaration_data.areas_foret_communes.includes(area.id_parent);
                });
                // Met à jour areas_foret_communes pour ne garder que celles qui sont encore référencées comme parent
                this.declaration_data.areas_foret_communes = this.declaration_data.areas_foret_communes.filter(commune =>
      ...
      action_button_retour_arriere() {
        ...
        if (this.typeCarte == CADASTRES) {
          if (this.last_typeCarte == SECTIONS) {
            this.last_typeCarte = COMMUNES;
            this.typeCarte = SECTIONS; // On revient au mode communes
          ...
          } else if (this.last_typeCarte == FORETS_DGD){ // on était des cadastres de forêts DGD on retourne vers la foret
        ...
        }else if (this.typeCarte == FORETS_DGD || this.typeCarte == FORETS_ONF || this.typeCarte == COMMUNES) {
          this.last_typeCarte = DEFAUT; // On revient au mode par défaut
        }else{
       ...
       */
       initMap() {
        const { INIT_VIEW, INIT_ZOOM, INIT_TILE, styles, baseTilesConfig, tileList, layers, configBaseSelect } = configMap;
      ...
      async fetch_new_geojson_data(id_type_carte, id_area_parent = null) {
        if (id_type_carte !== DEFAUT){
          ...
          if (id_area_parent && id_type_carte) { // Si il y a un id_area_parent, cela concerne l' affichage des sections ou des cadastres
      ...
      is_layer_in_cache(id_type_carte, id_area = null) {
        ...
        if (id_area) {
          return this.layers_groupe_cache[id_type_carte] && this.layers_groupe_cache[id_type_carte][id_area];
        } else {
      ...
      /**
       * Récupère les geojson des zones déja enregistrées dans la déclaration pour avoir le code de l'aire a afficher 
       * sur les pastilles à droite de la carte
       */
      fetch_areas_from_declaration_data() {
        let list_to_fetch;
        if (this.declaration_data.b_document === true && this.declaration_data.b_statut_public === true) {
          list_to_fetch = this.declaration_data.areas_localisation_onf_ug;
        } else {
          list_to_fetch = this.declaration_data.areas_localisation_cadastre;
        }

        if (!list_to_fetch || list_to_fetch.length === 0) {
          return;
        }
        fetch_areas_from_list_ids(list_to_fetch).then((data) => {
            if (data && data.features) {
                  data.features.forEach(feature => {
                  const index = this.declaration_data.areas_localisation.findIndex(area => area.id_area === feature.properties.id_area);
                  if (index !== -1) {
                    // Remplace l'élément existant
                    this.declaration_data.areas_localisation.splice(index, 1, { id_area: feature.properties.id_area, ...feature.properties });
                  } else {
                    // Ajoute si non existant
                    this.declaration_data.areas_localisation.push({ id_area: feature.properties.id_area, ...feature.properties });
                  }
                });
            }
        }).catch((error) => {
      ...
      async initialise_areas(){
        if (this.declaration_data.areas_localisation && this.declaration_data.areas_localisation.length > 0) {
          ...
          this.hierarchy_areas = await fetch_hierarchy_areas(this.list_selected_areas); // Récupère la hiérarchie des zones sélectionnées
          this.declaration_data.areas_localisation = []; // Réinitialise les zones sélectionnées pour les remplir à nouveau
        }else {
        ...
        }else if (this.declaration_data.b_document === true && this.declaration_data.b_statut_public === false) {
