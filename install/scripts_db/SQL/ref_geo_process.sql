...
INSERT INTO ref_geo.l_areas(id_type, area_name, area_code, geom, centroid, source, comment, enable)
    SELECT ref_geo.get_id_type(CONCAT('OEASC_ONF_FRT', '')), CONCAT(dept,'-',llib_frt), CONCAT(dept,'-',ccod_frt), geom, ST_CENTROID(geom), 'OEASC', '', true
    FROM ref_geo.l_OEASC_ONF_FRT;


INSERT INTO ref_geo.l_areas(id_type, area_name, area_code, geom, centroid, source, comment, enable)
    SELECT ref_geo.get_id_type(CONCAT('OEASC_ONF_PRF', '')), CONCAT(ccod_prf), CONCAT(dept,'-',ccod_frt,'-',ccod_prf), geom, ST_CENTROID(geom), 'OEASC', '', true
    FROM ref_geo.l_OEASC_ONF_PRF;


INSERT INTO ref_geo.l_areas(id_type, area_name, area_code, geom, centroid, source, comment, enable)
    SELECT ref_geo.get_id_type(CONCAT('OEASC_ONF_UG', '')), CONCAT(ccod_prf,'-',ccod_ug,'_',suffix), CONCAT(dept,'-',ccod_frt,'-',ccod_prf,'-',ccod_ug,'-',suffix), geom, ST_CENTROID(geom), 'OEASC', '', true
    FROM ref_geo.l_OEASC_ONF_UG;


INSERT INTO ref_geo.l_areas(id_type, area_name, area_code, geom, centroid, source, comment, enable)
    SELECT ref_geo.get_id_type(CONCAT('OEASC_DGD', '')), CONCAT(forinsee,'-',fornom), CONCAT(forid,'-',proref), geom, ST_CENTROID(geom), 'OEASC', '', true
    FROM ref_geo.l_OEASC_DGD;


INSERT INTO ref_geo.l_areas(id_type, area_name, area_code, geom, centroid, source, comment, enable)
    SELECT ref_geo.get_id_type(CONCAT('OEASC_CADASTRE', '')), CONCAT(insee_com,'-',section,'-',num_parc), CONCAT(insee_com,'-',section,'-',num_parc), geom, ST_CENTROID(geom), 'OEASC', '', true
