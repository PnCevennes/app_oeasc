$(document).ready(function() {
  ...
  var table = $("#table_declarations").DataTable({
    columnDefs: [
      ...
      {
        targets: [table_indices["Parcelle(s)"]],
        width: "100px"
      },
      {
  ...
  $("#table_declarations tbody tr").on("click", function() {
    ...
    if ($this.hasClass("tr-selected")) {
      ...
      M["map_show_declarations"] &&
        M["map_show_declarations"].markers.eachLayer(function(l) {
  ...
  table.on("search.dt", function() {
    ...
    if (
      M["map_show_declarations"] &&
      M["map_show_declarations"].markers &&
      M["map_show_declarations"].markers._layers
    ) {
      ...
      M["map_show_declarations"] &&
        M["map_show_declarations"].markers_save.forEach(function(e) {
  ...
  var set_tab_declaration = function(id_declaration) {
    ...
    */
    return new Promise((resolve, reject) => {
      var selector =
        "#declarations_tabs a[href$=declaration_" +
        id_declaration +
        "_container]";
      ...
      $("#declarations_tabs").append(
        '<a class="nav-item nav-link" href="#declaration_' +
          id_declaration +
          '_container" data-toggle="tab" data-type="TC">\
        Déclaration ' +
          id_declaration +
      ...
      $("#declarations").append(
        '<div class="tab-pane active" id="declaration_' +
          id_declaration +
          '_container">Chargement en cours</div>'
      );
      $.ajax(
        "/api/declaration/declaration_html/" +
          id_declaration +
          "?btn_action=1&map_display=1"
      )
        .done(response => {
          $("#declaration_" + id_declaration + "_container").html(response);
          $('[data-toggle="tooltip"]').tooltip();
          var declaration = JSON.parse(
            $("#declaration_" + id_declaration).attr("data-declaration")
          );
        ...
        .fail(response => {
          $("#declaration_" + id_declaration).html(
            "<div>Fail</div>" + response
          );
        });
