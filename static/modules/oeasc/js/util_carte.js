 ...
 */
  ...
  $(document).ready(() => {
    ...
    var setBounds = (bounds, map, d = 1e3) => {
  ...
  var wait_for_map_loaded = function (map) {
    return new Promise((resolve) => {
      Promise.all(map.promises).then(() => {
        setTimeout(() => {
          map.promises = map.promises.filter((p) => !p.resolved);
...
return true;
  ...
  var remove_promise_on_tile = function (map) {
    var p_tile = map.promises.find((e) => e.type == 'tile')
    ...
    map.base_maps[map.cur_tile_name].on('tileload', (e) => {
      // Map.on('loading', (e) => {
        var p = new Promise((resolve) => {
          map.base_maps[map.cur_tile_name].on('load', (e) => {
...
hide();
    ...
    L.control.betterscale({ imperial: false,
...
return div;
    ...
    if (config && config.type && config.type.includes('perimetre')) {
      ...
      if (config && config.zoom && config.zoom.includes('perimetre')) {
        l_perimetre_OEASC.on('data:loaded', function () {
 map.fitBounds(this.getBounds());
  ...
  var add_titre = function (titre, map) {
    $(`#${map.id}`).parent().
prepend(`<div class='map-titre'><div>${titre}</div></div>`)
  };
...
return color.match(/[\.\d]+/g).map((a) => Number(a));
        }
      }
      ...
      var p = new Promise((resolve) => {
        ...
        $.ajax({
          ...
          success (response) {
            ...
            featuresCollection.eachLayer((layer) => {
              ...
              // Legende
              var cv = colorValues(color).map((a) => `${a}`)
              cv[3] = '0.5';
    ...
    var s_tooltip_degats = function (degats) {
      ...
      degats.forEach((degat) => {
        ...
        degat.degat_essences.forEach((degat_essence) => {
          ...
          if (!degat_essence || !degat_essence.degat_gravite_code) {
 return;
}
...
return s_popup;
 ...
 return;
  ...
  for (var d of declarations) {
    console.log(d.id_declaration, d.centroid)
  }
    ...
    for (k = 0; k < declarations.length; k++) {
      ...
      var { id_declaration } = declaration;
      var s_popup = s_popup_declaration(declaration);
      var { centroid } = declaration;
      var marker;
      ...
      L.setOptions(marker, { color: 'black',
radius: 100,
pane: 'PANE_5' });
  ...
  var add_secteurs = function (map, b_zoom = false) {
    var p = new Promise((resolve) => {
      ...
      var l_secteurs_OEASC = new L.GeoJSON.AJAX('/api/ref_geo/areas_simples_from_type_code/l/OEASC_SECTEUR', {
      ...
      l_secteurs_OEASC.on('data:loaded', () => {
        if (b_zoom) {
